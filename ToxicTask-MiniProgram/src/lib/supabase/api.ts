...
export const profileApi = {
  ...
  async getCurrentProfile(): Promise<Profile | null> {
    ...
    console.error('[Profile] Profile not found after retries');
    return null;
  },
  ...
  async updateDignityCoins(userId: string, amount: number): Promise<boolean> {
    ...
    if (error) {
      console.error('[Profile] Error updating dignity coins:', error);
      return false;
    }
...
export const shameLogApi = {
  ...
  async getAllShameLogs(): Promise<ShameLog[]> {
    ...
    if (error) {
      console.error('[ShameLog] Error fetching shame logs:', error);
      return [];
    }
  ...
  async createShameLog(shameLog: Omit<ShameLog, 'id' | 'created_at'>): Promise<ShameLog | null> {
    ...
    if (error) {
      console.error('[ShameLog] Error creating shame log:', error);
      return null;
    }
  ...
  async getUserShameLogs(userId: string): Promise<ShameLog[]> {
    ...
    if (error) {
      console.error('[ShameLog] Error fetching user shame logs:', error);
      return [];
    }
